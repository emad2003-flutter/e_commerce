...
import 'package:e_commerce/data/models/card_items_response_dto.dart';
  ...
  Future<Either<Failure, AddToCardResponseEntity>> addProductToCart(
    ...
    try {
      ...
      if (connectivityResult.contains(ConnectivityResult.wifi) ||
        ...
        if (response.statusCode! >= 200 && response.statusCode! < 300) {
          print(addToCardResponse);
          return Right(addToCardResponse);
        } else {
          return Left(
            ServerFailure(addToCardResponse.message ?? "Server Failure"),
          );
        }
      } else {
        return Left(NetworkFailure("No Internet Connection"));
